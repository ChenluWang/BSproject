<template>
  <div class="container">
    <h1 align="center">Create Your Task</h1>
    <div class="row">
      <div class="col-md-4">
...
export default {
  name: 'CreateVedioTask',  //this is the name of the component
  data () {
    return {
  ...
  methods: {
    onSubmit () {
      ...
      const path = `/newtask_test/${user_id}`
      const payload = {
    ...
    upload(e){
        ...
        for(let item of e.target.files){
            ...
            if ((!/image\/\w+/.test(item.type)) && (!/video\/\w+/.test(item.type))){
                // 提示只能是图片，return
                alert("只能选择图片");
                return;
            }
            if(/image\/\w+/.test(item.type)){
                ...
                const aBlob = new Blob([item],{type:'video/mp4'})
                const reader = new FileReader();
                reader.onload=(ev)=>{
                    this.video.push(ev.target.result)
                    console.log(this.video)
                }
